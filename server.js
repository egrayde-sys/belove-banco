...
function checkRateLimit(ip) {
  ...
  const maxRequests = 60; // más permisivo porque no hay Claude
...
<title>BeLove Banco — Login</title>
<style>
...
<div class="card">
  <h1>🏦 BeLove Banco</h1>
  <p>Control de movimientos bancarios</p>
...
const server = http.createServer((req, res) => {
  ...
  if (!checkRateLimit(ip)) {
    ...
    res.end('Demasiadas solicitudes. Intenta más tarde.');
    return;
  }
...
server.listen(PORT, () => {
  console.log(`✅ BeLove Banco corriendo en puerto ${PORT}`);
});
